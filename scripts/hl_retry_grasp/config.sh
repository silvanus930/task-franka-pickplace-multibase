#!/usr/bin/env bash
# HL-only retry + grasp-hold tweak — eval with frozen 5510 checkpoint (no training).
set -euo pipefail
...
EVAL_RETRY_GRASP_CONFIG="${EVAL_NAV}/configs/task-franka-pickplace-multibase-retry-grasp.yaml"
