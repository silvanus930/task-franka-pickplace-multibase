...
class PickPlacePlanner:
    ...
    def __init__(
        ...
        *,
        ...
        import os as _os
        self.grasp_z_bias = float(_os.environ.get("NEPHER_GRASP_Z_BIAS", "0.03"))
        ...
        self.retry_depth_boost = float(_os.environ.get("NEPHER_RETRY_DEPTH", "0.012"))
        ...
        # ---- MUSTARD TIP-OVER (env-gated NEPHER_MUSTARD_TIP, default ON) ----
        ...
        self.mustard_tip   = _os.environ.get("NEPHER_MUSTARD_TIP", "1") != "0"
    ...
    def step(
        ...
        if _osd.environ.get("NEPHER_DIAG"):
            ...
            longstuck = in_desc & (self._elapsed > self.stall_time_s)
            if longstuck.any():
                ...
                print(f"[DESCEND-STALL] n={int(_ls.sum())} pos_err mean={pe.mean():.3f} | "
                      ...
                      f"elapsed>time={int((_ls&(self._elapsed>self.stall_time_s)).sum())} "
        ...
        if self.num_objects > 1:
            ...
            if advance_task.any():
                ...
                if self.via_home:
                    self._via_home[advance_task] = True  # route through home config before next grasp
                ...
                self._tip_phase[advance_task]      = 0
                self._tip_dir[advance_task]        = 0.0
    ...
    def _end_pose(
        ...
        z_grasp   = cz + per_z + H - self.grasp_z_bias - ramp - retry_boost   # per-object grasp depth
        ...
        if self.container_drop:
            ...
            z_release = gz + self.release_z_offset + H + self.drop_z_bonus + late
        else:
            z_place   = gz + per_z + H
        ...
        if self.container_drop:
            if self.retract_clear_z > 0.0 and getattr(self, "_cur_ee_pos", None) is not None:
                cleared = (self._cur_ee_pos[:, 2] - gz) > self.retract_clear_z
            else:
                cleared = torch.ones_like(gx, dtype=torch.bool)
            shift = is_retract & cleared
            # Until cleared: hold XY over the drop point (gx_c, gy_c) -> pure vertical lift.
            ex = torch.where(is_retract & ~cleared, gx_c, ex)
            ey = torch.where(is_retract & ~cleared, gy_c, ey)
            ex = torch.where(shift, gx + self._retract_xy_off[0], ex)
            ey = torch.where(shift, gy + self._retract_xy_off[1], ey)
