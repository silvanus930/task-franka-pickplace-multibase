...
def reset_typed_objects_from_scenario(
    ...
    * Overrides the episode goals with per-slot container drop positions.
    ...
    import os as _os
    # Default pick order = "farcube": farthest-from-bin first, but promote the
    # tiny DexCube before arm configuration degrades on later grasps. The base
    # "far" order cuts early table knock-offs (39->8 events) and bin-shove
    # (25->17) vs the natural slot order, while "farcube" keeps that ordering
    # and protects the depth-sensitive cube. Set
    # NEPHER_PICK_ORDER=none to disable (natural order) for comparison.
    _po = _os.environ.get("NEPHER_PICK_ORDER", "farcube")
    if _po == "none":
        _po = None
    if _po in ("low", "tall"):
