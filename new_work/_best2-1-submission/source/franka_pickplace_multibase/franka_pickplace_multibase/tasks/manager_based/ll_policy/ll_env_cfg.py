...
class RewardsCfg:
    ...
    # ---- Position tracking ----
    # ENV5-PRESERVE FINETUNE: coarse far-field pull boosted -0.5 -> -2.0. The fine
    # (std 0.05) term saturates ~5cm and ultrafine (std 0.02) gives ZERO gradient
    # beyond 2cm, so a precision-tuned policy gets "satisfied" close-but-short and
    # lacks the pull to force env5's hard 6.5cm low reach (z=0.083). A strong coarse
    # L2 pull restores far's aggressiveness WHILE fine/ultrafine keep grasp precision.
    ...
    # ULTRAFINE position: sub-2cm gradient (the existing fine term saturates at
    # ~5cm so it gives no signal below that = the grasp-zone precision gap). std
    # 0.02 keeps pushing the EE to <1cm. Paired with an ultrafine ORIENTATION term
    # (below) so the precision push does NOT blow out orientation like past retrains.
    # PLACEMENT-PRECISION FINETUNE: ultrafine 0.3->0.8 for tighter sub-2cm arrival
    # (cleaner drops that don't clip/shove the light bin = our top failure, ~17 eps).
    ee_pos_tracking_ultrafine = RewTerm(
        func=mdp.position_command_error_tanh,
        weight=0.8,
        params={
            "std": 0.02,
    ...
    # HIGH-PRECISION (80% goal): sub-cm gradient. std 0.01 gives signal only inside
    # ~1cm -> drives the EE to <5mm, which is what reliable grasps / clean placement
    # (no bin-shove) / not-clipping tall objects all need. Coarse -2.0 still preserves
    # the env5 hard reach; this only sharpens the final approach.
    ee_pos_tracking_ultra2 = RewTerm(
        func=mdp.position_command_error_tanh,
        weight=1.5,
        params={
            "std": 0.01,
    ...
    # ULTRAFINE orientation: holds/sharpens the gripper-down pose to ~2deg while
    # the ultrafine POSITION term pushes <1cm. This pairing is the fix for the
    # past failure (position push alone blew orientation to ~16deg -> tilted grasp).
    ee_ori_tracking_ultrafine = RewTerm(
        func=mdp.orientation_command_error_tanh,
        weight=1.0,
        params={
            "std": 0.05,
    ...
    # HIGH-PRECISION ori (paired with pos ultra2): std 0.02 (~1deg) holds the gripper
    # dead-vertical as the position term drives sub-cm, so the precise grasp stays
    # axis-aligned (past precision pushes tilted the wrist -> tilted/failed grasps).
    ee_ori_tracking_ultra2 = RewTerm(
        func=mdp.orientation_command_error_tanh,
        weight=1.0,
        params={
            "std": 0.02,
    ...
    # FINETUNE FIX: start at the CONVERGED curriculum weights (-0.03/-0.005), not
    # the base (-0.01/-0.001). The original policy was trained smooth via curriculum
    # over thousands of iters; a short precision-heavy finetune at the LOW base
    # weights made the policy JERKY -> it knocked objects + caused planner timeouts
    # (incompatible with the planner). High smoothness from the start keeps it smooth
    # AND precise. Env-tunable for balance.
    action_rate = RewTerm(func=mdp.action_rate_l2,
                          weight=-0.03)
    # PLACEMENT-PRECISION FINETUNE: joint_vel -0.005->-0.012 for gentler arrival
    # (slower deceleration into targets -> less overshoot/impact that nudges the bin).
    joint_vel = RewTerm(
