...
def objects_in_container(
    ...
    * Condition held for ``success_dwell_s`` seconds continuously.
    ...
    all_in = grip_ok
    count_in = torch.zeros(env.num_envs, dtype=torch.long, device=env.device)
    ...
    # Honest progress proxy: per-episode MAX number of active objects in the
    # LIVE container (ignoring grip). Banked per env on episode reset; the final
    # distribution is dumped via the diag summary at eval end.
    import os as _os
    if _os.environ.get("NEPHER_DIAG"):
        N = env.num_envs
        ep_len = env.episode_length_buf
        if not hasattr(env, "_diag_inbin_max"):
            env._diag_inbin_max = torch.zeros(N, dtype=torch.long, device=env.device)
            env._diag_inbin_prev_len = ep_len.clone()
            env._diag_inbin_log = []
        reset_now = ep_len <= env._diag_inbin_prev_len  # counter went back => new episode
        banked = reset_now & (env._diag_inbin_prev_len > 2)
        if banked.any():
            env._diag_inbin_log.extend(env._diag_inbin_max[banked].tolist())
            lg = env._diag_inbin_log
            hist = {k: lg.count(k) for k in range(6)}
            mean = sum(lg) / max(len(lg), 1)
            print(f"[INBIN] episodes={len(lg)} mean_in_bin={mean:.3f} hist(0..5)={hist}", flush=True)
        env._diag_inbin_max = torch.where(
            reset_now, count_in, torch.maximum(env._diag_inbin_max, count_in)
        )
        env._diag_inbin_prev_len = ep_len.clone()
...
def any_object_fell(
    ...
    fell_obj = torch.full((env.num_envs,), -1, dtype=torch.long, device=env.device)
    for ci, cfg in enumerate(object_cfgs):
        obj = env.scene[cfg.name]
        local_z = obj.data.root_pos_w[:, 2] - env_origin_z
        this = local_z < minimum_height
        fell_obj = torch.where(this & (fell_obj < 0), torch.full_like(fell_obj, ci), fell_obj)
        fell = fell | this

    import os as _os
    if _os.environ.get("NEPHER_DIAG") and fell.any():
        _ensure_log_configured()
        try:
            from .commands import HLPoseCommand
            pt = env.command_manager.get_term("ee_pose")
            stage = pt.planner.stage
            tidx = pt.planner._task_idx
        except Exception:
            stage = tidx = None
        for i in torch.where(fell)[0].tolist():
            st = int(stage[i].item()) if stage is not None else -1
            ti = int(tidx[i].item()) if tidx is not None else -1
            ci = int(fell_obj[i].item())
            print(f"[HL] env {i} CUBE_FELL catalog_{ci} stage={st} task_idx={ti} -> reset", flush=True)
    return fell
...
def object_dropped_mid_carry(
    ...
    * **Retries remaining** – the planner's own carry-miss recovery resets to
    ...
    if getattr(planner, "skip_then_revisit", False):
        retries_exhausted = retries_exhausted & ~planner._object_deferred[arange, task_idx]
...
def container_displaced(
    ...
    * **Yaw rotation** — absolute yaw difference between current and home
    ...
    # Tunable overrides (env-var) for measured recalibration of this tripwire.
    # The 2cm/0.1rad defaults fire on normal placement physics (a dropped object
    # nudges the dynamic bin 2-3cm); success still requires objects in the LIVE bin.
    # NOTE: thresholds left at the task's original values (do NOT relax these —
    # the displacement termination is an evaluation/pass-fail condition, not the
    # agent's control logic).
    max_displacement = 0.02
    max_yaw_displacement = 0.1
    ...
    if enable_log and displaced.any():
        ...
        _stg = _tix = None
        try:
            _pt = env.command_manager.get_term("ee_pose")
            _stg = _pt.planner.stage; _tix = _pt.planner._task_idx
        except Exception:
            pass
        for i in torch.where(displaced)[0].tolist():
            ...
            _sinfo = f" stage={int(_stg[i].item())} task_idx={int(_tix[i].item())}" if _stg is not None else ""
            msg = (
